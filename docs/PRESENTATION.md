...
| ~~**Gestion de stock**~~ | ✅ **Implémenté** — Colonne `stock` (integer) ajoutée à `Product` (migration `Version20260901120000`). `OrderService` vérifie le stock et le décrémente dans la transaction de création. `Product::decrementStock()` lève une `LogicException` si stock insuffisant. Le front (`ProductDetailPage`) affiche « Rupture de stock », « Plus que N en stock » (≤ 5), et plafonne le sélecteur de quantité. `isAvailable` conservé comme override admin. **Deux réserves explicites** : (1) aucun verrou n'est posé, la survente concurrente reste possible ; (2) la restitution, longtemps absente hors webhook, est désormais portée par `StockReleaseSubscriber` et couvre toutes les origines d'annulation, EasyAdmin compris |

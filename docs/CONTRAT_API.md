...
- **Le webhook Stripe.** `POST /api/webhooks/stripe` est **implémenté** dans `WebhookController`. Il est exempté du firewall (`PUBLIC_ACCESS` dans `security.yaml`) et du CSRF (`CsrfProtectionSubscriber::EXEMPT_PATHS`). Sa protection repose exclusivement sur la **vérification de signature HMAC** du payload via `Stripe\Webhook::constructEvent()`. Le webhook traite `payment_intent.succeeded` (Payment → CAPTURED, Order → PAID ; **commande déjà annulée → remboursement automatique** et alerte administrateur) avec idempotence. `payment_intent.payment_failed` ne modifie **rien** depuis le 14/09/2026 : chez Stripe, un refus laisse le paiement ouvert et le client réessaie — le marquer `FAILED`, comme auparavant, faisait ignorer le succès suivant et laissait un client débité avec une commande impayée ([CORRECTION.md](CORRECTION.md)). Il envoie un email de confirmation via `OrderConfirmationService` (best-effort).

...
>
> **Mise à jour du 14/09/2026 : la baseline est descendue à 76 entrées.** L'audit a corrigé la validation des entrées de l'API (`App\Http\JsonBody`, types stricts dans `AuthController`, `ProductService`, `OrderService`). Ces corrections ont fait disparaître **26 erreurs de typage que la baseline masquait** (25 lors de l'audit, 1 lors de la réorganisation de la création de commande) — PHPStan les a signalées comme entrées obsolètes. La baseline n'a été régénérée qu'après avoir vérifié qu'il ne restait **aucune** erreur réelle, pour ne pas y enfouir de nouveau défaut. Une baseline qui rétrécit mesure un progrès réel ; une baseline régénérée à l'aveugle ne mesure rien.
>
